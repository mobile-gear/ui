...
describe("orderSlice branch coverage", () => {
  ...
  describe("createOrder", () => {
    it("handles AxiosError rejection", async () => {
      ...
      await store.dispatch(createOrder({ items: [], totalAmount: 0, paymentIntentId: "pi_x", status: "pending", shippingAddress: mockAddress }));
      expect(store.getState().orders.error).toBe("Order failed");
    });

    it("sets pending state", async () => {
      let resolve: (v: unknown) => void;
      mockedService.create.mockReturnValue(new Promise((r) => { resolve = r; }));
      const store = createStore();
      const p = store.dispatch(createOrder({ items: [], totalAmount: 0, paymentIntentId: "pi_x", status: "pending", shippingAddress: mockAddress }));
      expect(store.getState().orders.isLoading).toBe(true);
      expect(store.getState().orders.error).toBeNull();
      resolve!({ id: 1, userId: 1, items: [], total: 0, paymentIntentId: "pi_x", status: "pending", shippingAddress: mockAddress, createdAt: "" });
      await p;
    });
  });
  ...
  describe("fetchUserOrders", () => {
    it("handles AxiosError rejection", async () => {
      mockedService.getUserOrders.mockRejectedValue(makeAxiosError("Fetch failed"));
      const store = createStore();
    ...
    it("sets pending state", async () => {
      let resolve: (v: unknown) => void;
      mockedService.getUserOrders.mockReturnValue(new Promise((r) => { resolve = r; }));
      const store = createStore();
  ...
  describe("updateOrderStatus", () => {
    it("handles AxiosError rejection", async () => {
      mockedService.updateStatus.mockRejectedValue(makeAxiosError("Update failed"));
      const store = createStore();
    ...
    it("handles update for non-existent order", async () => {
      const mockOrder = { id: 99, userId: 1, items: [], total: 0, paymentIntentId: "pi", status: "shipped" as const, shippingAddress: mockAddress, createdAt: "" };
      mockedService.updateStatus.mockResolvedValue(mockOrder);
      const store = createStore();
      await store.dispatch(updateOrderStatus({ orderId: 99, status: "shipped" }));
